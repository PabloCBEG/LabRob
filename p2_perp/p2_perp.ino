...
//protoripos de las funciones de actuacion
void palante(int);
void patras(int);
void parar(void);
...
void palante(int vel)
{
  ...
  analogWrite(ENA,vel+4);   //valor mayor en ENA, porque la rueda está un poco descompensada
  veld = vel;
...
void patras(int vel)
{
 ...
 analogWrite (ENA, vel);  //aquí parece que no hace falta el valor mayor, cuando el robot va hacia atrás no es tan pronunciada la descompensación
 veld = vel;
